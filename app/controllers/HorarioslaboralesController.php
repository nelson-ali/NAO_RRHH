...
class HorarioslaboralesController extends ControllerBase
{
    ...
    public function indexAction()
    {
        ...
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.tab.js?v=' . $version);
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.index.js?v=' . $version);
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.new.js?v=' . $version);
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.approve.js?v=' . $version);
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.edit.js?v=' . $version);
        $this->assets->addJs('/js/horarioslaborales/oasis.horarioslaborales.down.js?v=' . $version);
    }
        ...
        if ($resul->count() > 0) {
            foreach ($resul as $v) {
                ...
                $horariolaboral[] = array(
                    ...
                    'agrupador_descripcion' => $v->agrupador_descripcion,
    ...
    /**
     * Función para la carga del listado de horarios laborales disponibles en un rango de fechas para su uso en los calendarios laborales.
     * Se inhabilita la vista para el uso de jqwidgets,
     */
    public function listdisponiblesrangoAction()
    {
        $this->view->disable();
        $horariolaboral = Array();
        $obj = new Fhorarioslaborales();
        ...
        $mes = $_POST["mes"];
        $fechaIni = "01-" . (strlen($mes) == 1 ? "0" : "") . $mes . "-" . $gestion;
        $resul = $obj->getHorariosLaboralesDisponiblesRango(1, $fechaIni, null);
        //comprobamos si hay filas
        if ($resul->count() > 0) {
            foreach ($resul as $v) {
                switch ($v->estado) {
    ...
    public function saveAction()
    {
        ...
        if (isset($_POST["id"]) && $_POST["id"] > 0) {
            ...
            $agrupador = isset($_POST['agrupador'])?$_POST['agrupador']:0;
            $hoy = date("Y-m-d H:i:s");
            //$gestion_actual = date("Y");
            /*$fecha_ini = $hoy;
            $fecha_fin = "31/12/" . $gestion_actual;*/
            ...
            if ($nombre != '' && $color != '' && $hora_entrada != '' && $hora_salida != '' && $hora_inicio_rango_ent != '' && $hora_final_rango_ent != '' && $hora_inicio_rango_sal != '' && $hora_final_rango_sal != '') {
                ...
                if (count($objHorarioLaboral) > 0) {
                    $consulta = "(";
                    $consulta .= "fecha_ini BETWEEN CAST('" . $fechaIni . "' AS DATE) AND CAST('" . $fechaFin . "' AS DATE) ";
                    $consulta .= "OR ";
                    $consulta .= "fecha_fin BETWEEN CAST('" . $fechaIni . "' AS DATE) AND CAST('" . $fechaFin . "' AS DATE) ";
                    $consulta .= "OR ";
                    $consulta .= "CAST('" . $fechaIni . "' AS DATE) BETWEEN fecha_ini AND fecha_fin ";
                    $consulta .= "OR ";
                    $consulta .= "CAST('" . $fechaFin . "' AS DATE) BETWEEN fecha_ini AND fecha_fin ";
                    $consulta .= ")";
                    $cantMismoNombre = Horarioslaborales::count(array("id!=" . $_POST["id"] . " AND UPPER(nombre) LIKE UPPER('" . $nombre . "') AND " . $consulta));
                    if ($cantMismoNombre == 0) {
                        ...
                        $objHorarioLaboral->fecha_ini = $fechaIni;
                        $objHorarioLaboral->fecha_fin = $fechaFin;
                        ...
                        $objHorarioLaboral->agrupador = $agrupador;
                    ...
                    } else $msj = array('result' => 0, 'msj' => 'Error: Ya existe un horario con el mismo nombre y disponibilidad cruzada.');
                }
            } else {
            ...
            if ($nombre != '' && $color != '' && $hora_entrada != '' && $hora_salida != '' && $hora_inicio_rango_ent != '' && $hora_final_rango_ent != '' && $hora_inicio_rango_sal != '' && $hora_final_rango_sal != '') {
                ...
                $consulta .= "CAST('" . $fechaFin . "' AS DATE) BETWEEN fecha_ini AND fecha_fin ";
                $consulta .= ")";
                $cantMismoNombre = Horarioslaborales::count(array("UPPER(nombre) LIKE UPPER('" . $nombre . "') AND " . $consulta));
                if ($cantMismoNombre == 0) {
                    ...
                    $msj = array('result' => 0, 'msj' => 'Error: Ya existe un horario con el mismo nombre y disponibilidad cruzada.');
                }
            } else {
                $msj = array('result' => 0, 'msj' => 'Error: Los datos enviados no cumplen los criterios necesarios para su registro.');
            }
