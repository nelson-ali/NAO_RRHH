...
class PpostulantesController extends ControllerBase
{
	...
	public function indexAction()
	{
		...
		$this->assets
			...
			->addJs('/scripts/ppostulantes/exportar.js')
		;
	}
...
//                $query = $query . $where;
		...
		$model = new Ppostulantes();
		$resul = $model->lista($where,$orderby);
	...
	public function exportexcelAction($n_rows, $columns, $filtros, $groups, $sorteds)
	{
		...
		if (count($widthsSelecteds) > 0) {
			...
			$excel->debug = 0;
			$excel->title_rpt = utf8_decode('Reporte Postulantes');
...
//			$resul = $obj->lista(0, $where, $groups);
	...
	public function exportpdfAction($n_rows, $columns, $filtros, $groups, $sorteds)
	{
		...
		$generalConfigForAllColumns = array(
			...
			'grado' => array('title' => 'Grado', 'width' => 30, 'align' => 'C', 'type' => 'varchar'),
			...
			'correo' => array('title' => 'Correo', 'width' => 20, 'align' => 'L', 'type' => 'varchar'),
