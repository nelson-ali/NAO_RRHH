...
class ProcesoscontratacionesController extends ControllerBase
{
    ...
    public function listseguimientoporidAction()
    {
        //$resul = Seguimientos::find(array('baja_logica=1','order' => 'id ASC'));
        $customers = array();
        if (isset($_GET["id"]) && $_GET["id"] > 0) {
            ...
            $resul = $pc->listseguimientoporid($_GET["id"]);
    ...
    public function addAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $pac_id = isset($_POST['pac_ids']) && $_POST['pac_ids'] != "" ? $_POST['pac_ids'] : "";
            if ($resul->save()) {
                if ($pac_id != "") {
...
<div class="block">
    ...
    public function carrerascargosAction($pc)
    {
        if ($this->request->isPost()) {
            $cargo_id = $this->request->getPost('cargo_id');
            $carrera_ids = $this->request->getPost('carrera_check');

            $robots = Pcarreracargos::find("cargo_id='$cargo_id'");

            foreach ($robots as $robot) {
                $robot->delete();
            }

            foreach ($carrera_ids as $v) {
                $model = new Pcarreracargos();
                $model->cargo_id = $cargo_id;
                $model->pcarrera_id = $v;
                $model->save();
            }
        ...
        $model = new Procesoscontrataciones();
        $cargos = $this->tag->select(
            array(
                'cargo_id',
                $model->getcargosprocesoscontratacion($pc),
        ...
        $model = Pcarreras::find("baja_logica=1");
        $this->view->setVar('carreras', $model);
    }
}
